...
from forge.gui.tool_icons import ICON_POINT_DRAW, ICON_POLY_DRAW, ICON_POLY_EDIT
from forge.io import fancy_json_string
from forge.utils import orthogonalised_convex_hull_from_rects

# Bokeh toolbar icons, shared with the Wall tab, used in sidebar help
# text so the user can identify the correct button.
_ICON_POINT_DRAW = ICON_POINT_DRAW
_ICON_POLY_DRAW = ICON_POLY_DRAW
_ICON_POLY_EDIT = ICON_POLY_EDIT
...
class GeometryTab:
    ...
    def clear_wall_dependent_geometry(self):
        """Clear geometry defined against the wall, across every region.

        Strike points are snapped onto the wall and buffers are built from
        specific wall segments, so both are meaningless once the wall changes.
        X-point regions are free-space polygons and are left alone.

        Returns
        -------
        bool
            True if anything was actually cleared.
        """
        self._save_active_region()

        had_data = any(
            d["strike"]["x"] or d["buffers"]
            for d in self._region_data.values()
        )
        if not had_data:
            return False
        ...
        self.status.object = (
            "Strike geometry and buffers were cleared for all regions because "
            "the machine wall changed. X-point regions were kept. Redefine "
            "strike geometry and buffers against the new wall."
        )
        self.status.alert_type = "warning"
        return True
